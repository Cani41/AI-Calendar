"use client";

import { useState } from "react";

export default function Home() {
  const [loading, setLoading] = useState(false);

  return (
    <div className="flex h-screen bg-[#05050a] text-white font-[450] justify-center items-center p-5">
      <div className="flex flex-col w-full max-w-[800px] h-full bg-[#0e0e16] border border-white/10 rounded-2xl overflow-hidden justify-center items-center gap-6 p-8">
        <div className="w-14 h-14 rounded-2xl bg-blue-600 flex items-center justify-center text-lg font-bold tracking-wide">
          AI
        </div>

        <div className="text-center">
          <h1 className="text-2xl font-semibold mb-2">Kalenteri-assistentti</h1>
          <p className="text-gray-400 text-[14px]">Hallitse kalenteriasi luonnollisella kielellä</p>
        </div>

        <a
          href="/api/auth/google"
          onClick={() => setLoading(true)}
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-[14px] transition-all ${
            loading
              ? "bg-blue-700 text-blue-300 cursor-wait"
              : "bg-blue-600 hover:bg-blue-500 text-white"
          }`}
        >
          {loading ? (
            <>
              <span className="flex gap-[4px] items-center">
                <span className="w-1.5 h-1.5 bg-blue-300 rounded-full animate-bounce [animation-delay:0ms]" />
                <span className="w-1.5 h-1.5 bg-blue-300 rounded-full animate-bounce [animation-delay:150ms]" />
                <span className="w-1.5 h-1.5 bg-blue-300 rounded-full animate-bounce [animation-delay:300ms]" />
              </span>
              Ohjataan Google-kirjautumiseen…
            </>
          ) : (
            <>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#fff"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff"/>
              </svg>
              Kirjaudu Google-tilillä
            </>
          )}
        </a>
      </div>
    </div>
  );
}
